(function() {
		...
		function spriteSheet(el, duration, elWidth, steps) {
			...
			function hover() {
				...
				sprite.to(el, duration, { backgroundPosition:lastPosition, ease:SteppedEase.config(steps - 1) });
			}
	
			$(el).addEventListener('mouseenter', hover);
	
			function skipReset() {
				sprite.to(el, duration, { backgroundPosition:"0px", ease:SteppedEase.config(steps - 1) });
			}
		...
		window.onload = function() {
			if (Enabler.isInitialized()) {
				...
				Enabler.addEventListener(studio.events.StudioEvent.INIT, doubleclick_init);
			}
			function doubleclick_init() {
				if (Enabler.isPageLoaded()) {
					polite_load();
				} else {
					Enabler.addEventListener(studio.events.StudioEvent.PAGE_LOADED, polite_load);
				}
			}
		...
		function banner() {
			...
			tl1.call(videoPlay)
			tl1.from('#frame1_tt', (dur * 4), {ease:Power2.easeOut, autoAlpha:0}, "frame1+=0.25")
			tl1.to('#frame1_clouds', 29.75, {ease: Power0.easeNone, x: -366}, "frame1+=0.25")
			tl1.from('#frame1_sub', (dur * 4), {ease:Power2.easeOut, autoAlpha:0}, "frame1+=0.25")
			tl1.from('#frame1_release', (dur * 4), {ease:Power2.easeOut, autoAlpha:0}, "frame1+=0.5")
			tl1.from('#cta_exit', (dur * 4), {ease:Power2.easeOut, autoAlpha:0}, "frame1+=0.75")
			tl1.from('#frame1_legal', (dur * 4), {ease:Power2.easeOut, autoAlpha:0}, "frame1+=1")
			tl1.from('#frame1_kanga', (dur * 6), {ease: Elastic.easeOut.config(2, 1.5), autoAlpha:0, x:-15, rotation:-15}, "frame1+=1.2")
			tl1.from('#frame1_piglet', (dur * 6), {ease: Elastic.easeOut.config(2, 1.5), autoAlpha:0, x:-15, rotation:-15}, "frame1+=1.4")
			tl1.from('#frame1_roo', (dur * 6), {ease: Elastic.easeOut.config(2, 1.5), autoAlpha:0, x:-15, rotation:-15}, "frame1+=1.6")
			tl1.from('#frame1_tigger', (dur * 6), {ease: Elastic.easeOut.config(2, 1.5), autoAlpha:0, x:-15, rotation:-15}, "frame1+=1.8")
			tl1.from('#frame1_pooh', (dur * 6), {ease: Elastic.easeOut.config(2, 1.5), autoAlpha:0, x:-15, rotation:-15}, "frame1+=2")
			tl1.from('#frame1_eeyohh', (dur * 6), {ease: Elastic.easeOut.config(2, 1.5), autoAlpha:0, x:-15, rotation:-15}, "frame1+=2.2")
			tl1.from('#frame1_rabbit', (dur * 6), {ease: Elastic.easeOut.config(2, 1.5), autoAlpha:0, x:-15, rotation:-15}, "frame1+=2.4")
			;
			...
			//log total duration
			if ( dbug ) {
				console.log('animation length = ' + parseFloat(Math.round(tl1.totalDuration() * 100) / 100).toFixed(2) + 's');
			}
